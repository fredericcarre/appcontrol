...
| Dependency Vulnerabilities (npm) | **PASS** | Critical: 0, High: 0 (1 moderate, non-exploitable) |
| Supply Chain (cargo-deny) | **PASS** | Advisory DB + trusted sources |
| Secret Scanning | **PASS** | 0 findings (false positive on config variable names excluded) |
...
- **Release build:** PASS
- **Lines of code:** ~89447 LoC across 8 crates
...
- **Build:** PASS
- **Lines of code:** ~31366 LoC, 121 components
...
- **Status:** PASS
- **Critical:** 0 (1 moderate — non-exploitable in production context)
- **High:** 0
- **Moderate:** 1
...
- **Status:** PASS (false positive on config variable names excluded)
...
- **Database migrations:** 90 SQL migration files
- **Binary checksums:** SHA-256 checksums published in `checksums-sha256.txt` with each release
- **Reproducible builds:** all binaries built from tagged commit `b491fd3` in CI
...
*It reflects the state of the codebase at commit `b491fd30af43a48f65feaaf0dc11a305b950e622` on 2026-04-14 05:27 UTC.*
