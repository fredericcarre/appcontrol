...
mod test_saml_auth {
    ...
    async fn test_saml_metadata_endpoint() {
        ...
        assert!(body.contains("EntityDescriptor"), "Should contain EntityDescriptor");
        assert!(body.contains("appcontrol-test-sp"), "Should contain SP entity ID");
        assert!(body.contains("AssertionConsumerService"), "Should contain ACS definition");
        assert!(body.contains("urn:oasis:names:tc:SAML:2.0"), "Should reference SAML 2.0");
    ...
    async fn test_saml_login_redirects_to_idp() {
        ...
        let resp = ctx.client_no_redirect()
    ...
    async fn test_saml_acs_processes_response() {
        ...
        let encoded = base64::engine::general_purpose::STANDARD.encode(saml_response_xml.as_bytes());

        // POST to ACS endpoint (form-encoded, as per SAML HTTP-POST binding)
        let resp = ctx.post_form_anonymous("/api/v1/auth/saml/acs", &[
        ...
        let body = resp.text().await.unwrap();
        assert!(body.contains("localStorage.setItem('token'"), "Should set JWT token in localStorage");
    ...
    async fn test_saml_group_mapping_crud() {
        ...
        // Create a team first
        let resp = ctx.post("/api/v1/teams", json!({
            "name": "Payments-Ops",
        ...
        let resp = ctx.post("/api/v1/saml/group-mappings", json!({
            "saml_group": "CN=APP_PAYMENTS_OPS,OU=Groups,DC=corp",
            "team_id": team_id,
            "default_role": "operator"
        })).await;
        assert_eq!(resp.status().as_u16(), 200, "Group mapping creation should succeed");
        let mapping: Value = resp.json().await.unwrap();
        assert_eq!(mapping["saml_group"], "CN=APP_PAYMENTS_OPS,OU=Groups,DC=corp");
        ...
        let found = mappings.iter().any(|m| m["saml_group"] == "CN=APP_PAYMENTS_OPS,OU=Groups,DC=corp");
        assert!(found, "Should find the created mapping");

        // Delete group mapping
        let mapping_id = mapping["id"].as_str().unwrap();
        let resp = ctx.delete_as("admin", &format!("/api/v1/saml/group-mappings/{mapping_id}")).await;
        assert_eq!(resp.status().as_u16(), 204, "Delete should return 204");

        // Verify deletion
        let resp = ctx.get("/api/v1/saml/group-mappings").await;
        let mappings: Vec<Value> = resp.json().await.unwrap();
        let found = mappings.iter().any(|m| m["id"].as_str() == Some(mapping_id));
    ...
    async fn test_saml_group_sync_adds_to_team() {
        ...
        ctx.post("/api/v1/saml/group-mappings", json!({
        ...
        let encoded = base64::engine::general_purpose::STANDARD.encode(saml_response_xml.as_bytes());
        ctx.post_form_anonymous("/api/v1/auth/saml/acs", &[
        ...
        assert!(is_member, "User should be added to the team via SAML group sync");
    ...
    async fn test_saml_group_removal_on_login() {
        ...
        let in_a = sqlx::query_scalar::<_, bool>(
            "SELECT EXISTS(SELECT 1 FROM team_members WHERE team_id = $1 AND user_id = $2)"
        ).bind(team_a_uuid).bind(user_id).fetch_one(&ctx.db_pool).await.unwrap();
        let in_b = sqlx::query_scalar::<_, bool>(
            "SELECT EXISTS(SELECT 1 FROM team_members WHERE team_id = $1 AND user_id = $2)"
        ).bind(team_b_uuid).bind(user_id).fetch_one(&ctx.db_pool).await.unwrap();
