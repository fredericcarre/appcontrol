...
mod test_websocket_events {
    ...
    async fn test_subscribe_receives_state_changes() {
        ...
        ws.send(Message::Text(subscribe_msg.to_string())).await.unwrap();
        ...
        if let Ok(Some(Ok(Message::Text(text)))) = msg {
            ...
            assert!(
                ...
                "Should receive a valid event type, got: {}", event["type"]
            );
        }
    ...
    async fn test_websocket_permission_filtering() {
        ...
        if let Ok(Some(Ok(Message::Text(text)))) = msg {
            ...
            if let Some(t) = event["type"].as_str() {
                assert!(t == "Error" || t == "PermissionDenied",
    ...
    async fn test_multiple_subscriptions() {
        ...
        let resp = ctx.post("/api/v1/apps", json!({"name": "Second-App"})).await;
