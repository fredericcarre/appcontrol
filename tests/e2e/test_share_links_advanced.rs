...
mod test_share_links_advanced {
    ...
    async fn test_share_link_with_expiry() {
        ...
        assert!(resp.status() == 401 || resp.status() == 403,
            "Expired share link should be rejected, got {}", resp.status());
    ...
    async fn test_share_link_with_max_uses() {
        ...
        let resp = ctx.get_anonymous(&format!("/api/v1/share/{token}")).await;
        assert!(resp.status() == 401 || resp.status() == 403 || resp.status() == 410,
            "Third use should be rejected (max_uses=2), got {}", resp.status());
    ...
    async fn test_share_link_revocation() {
        ...
        let resp = ctx.delete_as("admin",
            &format!("/api/v1/apps/{app_id}/permissions/share-links/{link_id}")).await;
        assert_eq!(resp.status(), 200);

        // Should no longer work
        let resp = ctx.get_anonymous(&format!("/api/v1/share/{token}")).await;
        assert!(resp.status() == 401 || resp.status() == 403 || resp.status() == 404,
            "Revoked share link should be rejected, got {}", resp.status());
    ...
    async fn test_list_share_links() {
        ...
        let resp = ctx.get_as("admin",
            &format!("/api/v1/apps/{app_id}/permissions/share-links")).await;
        assert_eq!(resp.status(), 200);
    ...
    async fn test_share_link_requires_manage_permission() {
        ...
        assert_eq!(resp.status(), 403, "Creating share links requires manage permission");
