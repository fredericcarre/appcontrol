...
async fn test_import_yaml_map() {
    ...
    sqlx::query("INSERT INTO sites (id, organization_id, name, code) VALUES ($1, $2, 'PRD', 'PRD')")
        .bind(site_id)
    ...
    assert!(oracle["check_cmd"].as_str().unwrap().contains("check_db.sh"));
    assert!(oracle["start_cmd"].as_str().unwrap().contains("start_db.sh"));
    ...
    // Verify dependencies
    let resp = ctx.get(&format!("/api/v1/apps/{app_id}/dependencies")).await;
    let body: Value = resp.json().await;
...
async fn test_import_creates_links() {
    ...
    let site_id = Uuid::new_v4();
    sqlx::query("INSERT INTO sites (id, organization_id, name, code) VALUES ($1, $2, 'PRD', 'PRD2')")
        .bind(site_id)
        .bind(ctx.organization_id)
        .execute(&ctx.db_pool)
        .await
    ...
    let resp = ctx.get(&format!("/api/v1/components/{oracle_id}/links")).await;
    assert_eq!(resp.status(), 200);
    let body: Value = resp.json().await;
    let links = body["links"].as_array().unwrap();
    assert_eq!(links.len(), 2);
    assert!(links.iter().any(|l| l["label"] == "Oracle Documentation" && l["link_type"] == "documentation"));
    assert!(links.iter().any(|l| l["label"] == "Grafana Dashboard" && l["link_type"] == "monitoring"));

    // Check apache links
    let apache_id = ctx.component_id(app_id, "apache-front").await;
    let resp = ctx.get(&format!("/api/v1/components/{apache_id}/links")).await;
    let body: Value = resp.json().await;
    let links = body["links"].as_array().unwrap();
...
async fn test_import_creates_command_params() {
    ...
    let site_id = Uuid::new_v4();
    sqlx::query("INSERT INTO sites (id, organization_id, name, code) VALUES ($1, $2, 'PRD', 'PRD3')")
        .bind(site_id)
        .bind(ctx.organization_id)
        .execute(&ctx.db_pool)
        .await
    ...
    let resp = ctx.get(&format!("/api/v1/commands/{deploy_cmd_id}/params")).await;
    let body: Value = resp.json().await;
    let params = body["params"].as_array().unwrap();
    assert_eq!(params.len(), 2);
    assert!(params.iter().any(|p| p["name"] == "env" && p["default_value"] == "prod"));
    assert!(params.iter().any(|p| p["name"] == "version" && p["validation_regex"] == r"^\d+\.\d+\.\d+$"));
...
async fn test_import_invalid_yaml() {
    ...
    let site_id = Uuid::new_v4();
    sqlx::query("INSERT INTO sites (id, organization_id, name, code) VALUES ($1, $2, 'PRD', 'PRD4')")
        .bind(site_id)
        .bind(ctx.organization_id)
        .execute(&ctx.db_pool)
        .await
...
async fn test_import_missing_dependency_warns() {
    ...
    let site_id = Uuid::new_v4();
    sqlx::query("INSERT INTO sites (id, organization_id, name, code) VALUES ($1, $2, 'PRD', 'PRD5')")
        .bind(site_id)
        .bind(ctx.organization_id)
        .execute(&ctx.db_pool)
        .await
...
"#;
    ...
    assert!(warnings[0].as_str().unwrap().contains("nonexistent-component"));
...
async fn test_import_audit_trail() {
    ...
    let site_id = Uuid::new_v4();
    sqlx::query("INSERT INTO sites (id, organization_id, name, code) VALUES ($1, $2, 'PRD', 'PRD6')")
        .bind(site_id)
        .bind(ctx.organization_id)
        .execute(&ctx.db_pool)
        .await
