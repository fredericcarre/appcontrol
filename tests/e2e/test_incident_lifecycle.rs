...
mod test_incident_lifecycle {
    ...
    async fn test_full_incident_detection_and_branch_restart() {
        ...
        assert_eq!(status.components.iter().filter(|c| c.state == "RUNNING").count(), 10,
        ...
        assert!(resp.status().is_success(),
            "start-branch should succeed, got {}", resp.status());
        ...
        for comp in &status.components {
            assert_eq!(comp.state, "RUNNING",
                "Component {} should be RUNNING after branch restart, got {}", comp.name, comp.state);
        }
        ...
        assert!(app1_transitions.iter().any(|t| t.from_state == "RUNNING" && t.to_state == "FAILED"),
            "state_transitions must record RUNNING → FAILED for App-1");

        // The recovery transitions should be recorded
        assert!(app1_transitions.iter().any(|t| t.to_state == "STARTING" || t.to_state == "RUNNING"),
        ...
        let branch_logs: Vec<_> = all_logs.iter()
            .filter(|l| l.action.contains("start") && l.resource_id == app_id)
            .collect();
        assert!(!branch_logs.is_empty(),
    ...
    async fn test_incident_with_multiple_failures_in_same_branch() {
        ...
        let resp = ctx.post(&format!("/api/v1/apps/{}/start-branch", app_id), json!({})).await;
        assert!(resp.status().is_success(),
            "start-branch should handle multiple failures, got {}", resp.status());

        ctx.wait_app_branch_running(app_id, Duration::from_secs(30)).await.unwrap();

        // All should be RUNNING again
        let status = ctx.get_app_status(app_id).await;
        let running_count = status.components.iter().filter(|c| c.state == "RUNNING").count();
        assert_eq!(running_count, 10, "All 10 components should be RUNNING after recovery");
    ...
    async fn test_incident_audit_trail_completeness() {
        ...
        // Restart branch
        ctx.post(&format!("/api/v1/apps/{}/start-branch", app_id), json!({})).await;
        ctx.wait_app_branch_running(app_id, Duration::from_secs(30)).await.unwrap();
        ...
        assert!(final_transition_count > initial_transition_count + 1,
        ...
        let app1_transitions = ctx.get_state_transitions_for(app_id, "App-1").await;
        let failed_transition = app1_transitions.iter()
            .find(|t| t.to_state == "FAILED")
            .expect("Must find FAILED transition for App-1");
        assert_eq!(failed_transition.from_state, "RUNNING",
            "Failed transition should be from RUNNING");
        assert_eq!(failed_transition.trigger, "check",
    ...
    async fn test_incident_does_not_cascade_to_separate_branches() {
        ...
        for name in ["DB-2", "App-2", "Front-2", "Queue-2", "Worker-2"] {
            ...
            assert!(!transitions.iter().any(|t| t.to_state == "STARTING" || t.to_state == "STOPPING"),
    ...
    async fn test_incident_reports_show_failure_data() {
        ...
        let oracle_incidents: Vec<_> = data.iter()
            ...
            .collect();
        assert_eq!(oracle_incidents.len(), 2,
            "Incidents report should show 2 Oracle-DB failures, got {}", oracle_incidents.len());
