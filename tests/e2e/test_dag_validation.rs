...
mod test_dag_validation {
    ...
    async fn test_cycle_detection_rejects_circular_dependency() {
        ...
        let resp = ctx.post(&format!("/api/v1/apps/{app_id}/dependencies"), json!({
            "from_component_id": apache_id,
            "to_component_id": oracle_id,
        })).await;
        assert_eq!(resp.status(), 409, "Circular dependency must be rejected");
        let body: Value = resp.json().await;
        assert!(body["error"].as_str().unwrap().to_lowercase().contains("cycle"),
    ...
    async fn test_self_dependency_rejected() {
        ...
        assert!(resp.status() == 400 || resp.status() == 409,
            "Self-dependency must be rejected, got {}", resp.status());
    ...
    async fn test_duplicate_dependency_rejected() {
        ...
        assert!(resp.status() == 409 || resp.status() == 400,
            "Duplicate dependency must be rejected, got {}", resp.status());
    ...
    async fn test_dependency_deletion_recalculates_dag() {
        ...
        let deps: Value = resp.json().await;
        ...
        let tomcat_to_apache = deps.as_array().unwrap().iter()
            .find(|d| {
        ...
        let resp = ctx.delete_as("admin", &format!("/api/v1/dependencies/{dep_id}")).await;
        assert_eq!(resp.status(), 200);

        // Now Apache-Front should be independent (level 0 in its own plan)
        let resp = ctx.post(&format!("/api/v1/apps/{app_id}/start?dry_run=true"), json!({})).await;
        let plan: Value = resp.json().await;
        let level0 = &plan["plan"][0];
        let level0_names: Vec<&str> = level0.as_array().unwrap().iter()
            .map(|c| c["name"].as_str().unwrap())
            .collect();
        assert!(level0_names.contains(&"Apache-Front"),
    ...
    async fn test_dry_run_plan_has_correct_levels() {
        ...
        let l2: Vec<&str> = levels[2].as_array().unwrap().iter()
            .map(|c| c["name"].as_str().unwrap()).collect();
    ...
    async fn test_cross_app_dependency_rejected() {
        ...
        let app2: Value = resp.json().await;
        let app2_id: Uuid = app2["id"].as_str().unwrap().parse().unwrap();
        ctx.post(&format!("/api/v1/apps/{app2_id}/components"), json!({
        ...
        let resp = ctx.post(&format!("/api/v1/apps/{app1_id}/dependencies"), json!({
            "from_component_id": oracle_id,
            "to_component_id": other_id,
        })).await;
        assert!(resp.status() == 400 || resp.status() == 404,
            "Cross-app dependency must be rejected, got {}", resp.status());
