...
mod test_org_isolation {
    ...
    async fn test_apps_invisible_across_orgs() {
        ...
        let app_ids: Vec<&str> = apps.as_array().unwrap().iter()
            .filter_map(|a| a["id"].as_str())
            .collect();
        assert!(!app_ids.contains(&app_id.to_string().as_str()),
    ...
    async fn test_cross_org_app_access_returns_404() {
        ...
        let resp = ctx.get_with_token(&org2_token, &format!("/api/v1/apps/{app_id}")).await;
    ...
    async fn test_cross_org_start_returns_404() {
        ...
        let resp = ctx.post_with_token(&org2_token,
            &format!("/api/v1/apps/{app_id}/start"), json!({})).await;
        assert_eq!(resp.status(), 404,
            "Cross-org start should return 404");
    ...
    async fn test_cross_org_component_access_denied() {
        ...
        let resp = ctx.get_with_token(&org2_token,
            &format!("/api/v1/components/{oracle_id}")).await;
        assert_eq!(resp.status(), 404);
    ...
    async fn test_teams_scoped_to_org() {
        let ctx = TestContext::new().await;
        let _team_id = ctx.create_team("Org1-Team", vec![ctx.operator_user_id]).await;
        ...
        let team_names: Vec<&str> = teams.as_array().unwrap().iter()
            .filter_map(|t| t["name"].as_str())
            .collect();
        assert!(!team_names.contains(&"Org1-Team"),
    ...
    async fn test_each_org_has_independent_apps() {
        ...
        let (_, _, org2_token) = ctx.create_second_org().await;
        let resp = ctx.post_with_token(&org2_token, "/api/v1/apps",
            json!({"name": "Org1-App"})).await;
        assert!(resp.status().is_success(),
