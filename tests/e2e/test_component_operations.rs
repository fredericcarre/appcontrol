...
mod test_component_operations {
    ...
    async fn test_single_component_start() {
        ...
        let resp = ctx.post_as("admin",
            &format!("/api/v1/components/{oracle_id}/start"), json!({})).await;
        assert!(resp.status().is_success(),
            "Single component start should succeed, got {}", resp.status());

        // Wait for Oracle-DB to be RUNNING
        tokio::time::sleep(Duration::from_secs(5)).await;
        let state = ctx.get_component_state(app_id, "Oracle-DB").await;
        assert!(state == "RUNNING" || state == "STARTING",
            "Oracle-DB should be STARTING or RUNNING, got {state}");

        // Other components should still be STOPPED
        let state = ctx.get_component_state(app_id, "Tomcat-App").await;
        assert_eq!(state, "STOPPED", "Tomcat-App should still be STOPPED");
    ...
    async fn test_single_component_stop() {
        ...
        let resp = ctx.post_as("admin",
            &format!("/api/v1/components/{oracle_id}/stop"), json!({})).await;
        assert!(resp.status().is_success());

        tokio::time::sleep(Duration::from_secs(5)).await;
        let state = ctx.get_component_state(app_id, "Oracle-DB").await;
        assert!(state == "STOPPED" || state == "STOPPING",
    ...
    async fn test_component_crud() {
        ...
        // Update component
        let resp = ctx.put(&format!("/api/v1/components/{comp_id}"), json!({
        ...
        // Delete component
        let resp = ctx.delete_as("admin", &format!("/api/v1/components/{comp_id}")).await;
        assert_eq!(resp.status(), 200);
    ...
    async fn test_component_delete_requires_edit_permission() {
        ...
        let resp = ctx.delete_as("operator", &format!("/api/v1/components/{oracle_id}")).await;
        assert_eq!(resp.status(), 403);

        // Editor can
        ctx.grant_permission(app_id, ctx.editor_user_id, "edit").await;
        let resp = ctx.delete_as("editor", &format!("/api/v1/components/{oracle_id}")).await;
        assert_eq!(resp.status(), 200);
    ...
    async fn test_list_components_for_app() {
        ...
        assert_eq!(resp.status(), 200);
        let components: Value = resp.json().await;
        assert_eq!(components.as_array().unwrap().len(), 5,
