...
mod test_permissions_sharing {
    ...
    async fn test_permission_levels_enforce_correctly() {
        ...
        // Viewer can GET app
        let resp = ctx.get_as("viewer", &format!("/api/v1/apps/{}", app_id)).await;
        assert_eq!(resp.status(), 200);

        // Viewer CANNOT start app
        let resp = ctx.post_as("viewer", &format!("/api/v1/apps/{}/start", app_id), json!({})).await;
        ...
        let resp = ctx.post_as("operator", &format!("/api/v1/apps/{}/start", app_id), json!({})).await;
        assert!(resp.status() == 200 || resp.status() == 202);

        // Operator CANNOT modify config
        let resp = ctx.put_as("operator", &format!("/api/v1/apps/{}", app_id), json!({"name": "hacked"})).await;
        assert_eq!(resp.status(), 403);
        ...
        let resp = ctx.put_as("editor", &format!("/api/v1/apps/{}", app_id), json!({"description": "updated"})).await;
        assert_eq!(resp.status(), 200);

        // Editor CANNOT start (edit != operate)
        let resp = ctx.post_as("editor", &format!("/api/v1/apps/{}/start", app_id), json!({})).await;
        assert_eq!(resp.status(), 403);
    ...
    async fn test_team_permission_grants_access_to_all_members() {
        ...
        let team_id = ctx.create_team("Prod-Team", vec![ctx.operator_user_id, ctx.viewer_user_id]).await;
        ...
        let resp = ctx.get_as("viewer", &format!("/api/v1/apps/{}/permissions/effective", app_id)).await;
    ...
    async fn test_effective_permission_is_max_of_direct_and_team() {
        ...
        // Direct: view
        ctx.grant_permission(app_id, ctx.operator_user_id, "view").await;
        ...
        let resp = ctx.get_as("operator", &format!("/api/v1/apps/{}/permissions/effective", app_id)).await;
        let eff: Value = resp.json().await;
        assert_eq!(eff["level"], "operate");
    ...
    async fn test_expired_permission_ignored() {
        ...
        ctx.grant_permission_with_expiry(app_id, ctx.operator_user_id, "operate",
    ...
    async fn test_share_link_grants_access() {
        ...
        let resp = ctx.post_as("admin", &format!("/api/v1/apps/{}/share-links", app_id), json!({
            "permission_level": "view",
            "label": "For COMEX",
            "expires_at": "2026-12-31T00:00:00Z"
        })).await;
        let link: Value = resp.json().await;
    ...
    async fn test_org_admin_has_implicit_owner() {
        ...
        assert_eq!(resp.status(), 200, "Org admin should have implicit owner access");
