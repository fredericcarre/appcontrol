...
mod test_reports {
    ...
    async fn test_incidents_report_returns_seeded_failures() {
        ...
        assert_eq!(data.len(), 4, "Should have 4 incidents (3 Oracle + 1 Tomcat), got {}", data.len());
        ...
        let oracle_incidents: Vec<_> = data.iter()
            .filter(|d| d["component_name"].as_str() == Some("Oracle-DB"))
            .collect();
        assert_eq!(oracle_incidents.len(), 3, "Should have 3 Oracle-DB incidents");

        let tomcat_incidents: Vec<_> = data.iter()
            .filter(|d| d["component_name"].as_str() == Some("Tomcat-App"))
            .collect();
        assert_eq!(tomcat_incidents.len(), 1, "Should have 1 Tomcat-App incident");

        // Verify each incident has a timestamp
        for incident in data {
            assert!(incident["at"].is_string(), "Incident should have a timestamp");
        }
    ...
    async fn test_switchovers_report_returns_seeded_phases() {
        ...
        assert_eq!(data.len(), 4, "Should have 4 switchover log entries, got {}", data.len());

        // Verify phases are present
        let logged_phases: Vec<&str> = data.iter()
            .map(|d| d["phase"].as_str().unwrap())
            .collect();
    ...
    async fn test_audit_report_validates_seeded_content() {
        ...
        assert!(data.len() >= 4,
            "Audit report should have at least 4 entries, got {}", data.len());

        // Verify the seeded actions are present
        let reported_actions: Vec<&str> = data.iter()
            .map(|d| d["action"].as_str().unwrap())
            .collect();
        assert!(reported_actions.contains(&"start"), "Audit should contain 'start' action");
        assert!(reported_actions.contains(&"stop"), "Audit should contain 'stop' action");
        assert!(reported_actions.contains(&"config_change"), "Audit should contain 'config_change'");
        assert!(reported_actions.contains(&"diagnose"), "Audit should contain 'diagnose'");

        // Verify each entry has user_id and resource_type
        for entry in data {
            assert!(entry["user_id"].is_string(), "Audit entry should have user_id");
            assert!(entry["resource_type"].is_string(), "Audit entry should have resource_type");
        }

        // Verify user_id matches the admin who created the entries
        let admin_entries: Vec<_> = data.iter()
            .filter(|d| d["user_id"].as_str() == Some(&ctx.admin_user_id.to_string()))
            .collect();
        assert!(admin_entries.len() >= 4, "All seeded entries should be from admin user");
    ...
    async fn test_audit_report_date_filtering() {
        ...
        let actions: Vec<&str> = data.iter()
            .map(|d| d["action"].as_str().unwrap())
            .collect();
        assert!(actions.contains(&"recent_action"), "Should include recent_action");
        assert!(!actions.contains(&"old_action"), "Should exclude old_action (outside date range)");
    ...
    async fn test_rto_report_returns_null_with_no_data() {
        ...
        assert_eq!(resp.status(), 200);
        let report: Value = resp.json().await;
        assert!(report["average_rto_seconds"].is_null(),
    ...
    async fn test_all_six_report_endpoints_respond() {
        ...
        let endpoints = [
            "availability", "incidents", "switchovers",
            "audit", "compliance", "rto",
        ];

        for endpoint in &endpoints {
            let resp = ctx.get(&format!("/api/v1/apps/{app_id}/reports/{endpoint}")).await;
            assert!(resp.status().is_success(),
                "Report endpoint '{}' should return success, got {}", endpoint, resp.status());
        }
