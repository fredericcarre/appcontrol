...
mod test_teams_crud {
    ...
    async fn test_list_teams() {
        ...
        let teams: Value = resp.json().await;
    ...
    async fn test_team_crud_lifecycle() {
        ...
        // Update
        let resp = ctx.put(&format!("/api/v1/teams/{team_id}"), json!({
            "name": "Updated-Team",
        ...
        // Delete
        let resp = ctx.delete_as("admin", &format!("/api/v1/teams/{team_id}")).await;
        assert_eq!(resp.status(), 200);
    ...
    async fn test_add_and_remove_team_members() {
        ...
        // Add member
        let resp = ctx.post(&format!("/api/v1/teams/{team_id}/members"), json!({
        ...
        assert_eq!(resp.status(), 200);
        let members: Value = resp.json().await;
        let member_ids: Vec<&str> = members.as_array().unwrap().iter()
            .map(|m| m["user_id"].as_str().unwrap())
            .collect();
        assert!(member_ids.contains(&ctx.operator_user_id.to_string().as_str()));

        // Remove member
        let resp = ctx.delete_as("admin",
            &format!("/api/v1/teams/{team_id}/members/{}", ctx.operator_user_id)).await;
        assert_eq!(resp.status(), 200);
    ...
    async fn test_team_permission_revoked_on_member_removal() {
        ...
        let team_id = ctx.create_team("Revoke-Team", vec![ctx.operator_user_id]).await;
        ctx.grant_team_permission(app_id, team_id, "operate").await;

        // Operator can start (via team)
        let resp = ctx.get_as("operator",
            &format!("/api/v1/apps/{app_id}/permissions/effective")).await;
    ...
    async fn test_team_with_expiry() {
        let ctx = TestContext::new().await;
        let app_id = ctx.create_payments_app().await;
        let team_id = ctx.create_team("Expiry-Team", vec![ctx.operator_user_id]).await;

        // Grant team permission with expiry in the past
        ctx.post_as("admin", &format!("/api/v1/apps/{app_id}/permissions/teams"), json!({
            "team_id": team_id,
        ...
        let resp = ctx.post_as("operator",
            &format!("/api/v1/apps/{app_id}/start"), json!({})).await;
        assert_eq!(resp.status(), 403, "Expired team permission should be denied");
