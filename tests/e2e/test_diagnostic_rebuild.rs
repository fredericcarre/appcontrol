...
mod test_diagnostic_rebuild {
    ...
    async fn test_diagnose_produces_correct_recommendations() {
        ...
        let resp = ctx.post(&format!("/api/v1/apps/{}/diagnose", app_id), json!({})).await;
    ...
    async fn test_rebuild_respects_dag_order() {
        ...
        let resp = ctx.post(&format!("/api/v1/apps/{}/rebuild", app_id), json!({
            "components": [
    ...
    async fn test_rebuild_blocked_by_protection() {
        ...
        ctx.put(&format!("/api/v1/components/{}/rebuild-protection", oracle_id),
            json!({ "protected": true })).await;

        // Try to rebuild Oracle → should be rejected
        let resp = ctx.post(&format!("/api/v1/apps/{}/rebuild", app_id), json!({
            "components": [{ "id": oracle_id, "action": "app_rebuild" }]
        })).await;
        assert_eq!(resp.status(), 409, "Rebuild of protected component should be rejected");
