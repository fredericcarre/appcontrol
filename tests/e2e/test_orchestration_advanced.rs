...
mod test_orchestration_advanced {
    ...
    async fn test_status_endpoint_structure() {
        ...
        let resp = ctx.get(&format!("/api/v1/orchestration/apps/{app_id}/status")).await;
        assert_eq!(resp.status(), 200);

        let status: Value = resp.json().await;
        assert!(status["components"].is_array(), "Status should have components array");
    ...
    async fn test_wait_running_returns_timeout() {
        ...
        assert!(
            ...
            || resp.status() == 408,
            "Should indicate timeout, got: {:?}", body
        );
    ...
    async fn test_stop_dry_run() {
        ...
        let resp = ctx.post(&format!("/api/v1/apps/{app_id}/stop?dry_run=true"), json!({})).await;
        assert_eq!(resp.status(), 200);
        ...
        assert!(status.components.iter().all(|c| c.state == "RUNNING"),
    ...
    async fn test_api_key_crud() {
        ...
        // Create API key
        let resp = ctx.post("/api/v1/api-keys", json!({
            "name": "Test-Key",
        ...
        let resp = ctx.delete_as("admin", &format!("/api/v1/api-keys/{key_id}")).await;
        assert_eq!(resp.status(), 200);
    ...
    async fn test_api_key_stop() {
        ...
        let api_key = ctx.create_api_key("Scheduler", vec!["start", "stop", "status"]).await;

        // Stop via API key
        let resp = ctx.post_with_api_key(&api_key,
            &format!("/api/v1/apps/{app_id}/stop"), json!({})).await;
        assert!(resp.status().is_success(),
            "API key with 'stop' action should be able to stop, got {}", resp.status());
    ...
    async fn test_orchestration_endpoints_via_api_key() {
        let ctx = TestContext::new().await;
        let app_id = ctx.create_payments_app().await;
        let api_key = ctx.create_api_key("Orchestrator", vec!["start", "stop", "status"]).await;

        // Orchestration status
        let resp = ctx.get_with_api_key(&api_key,
            &format!("/api/v1/orchestration/apps/{app_id}/status")).await;
        assert_eq!(resp.status(), 200);

        // Orchestration start
        let resp = ctx.post_with_api_key(&api_key,
            &format!("/api/v1/orchestration/apps/{app_id}/start"), json!({})).await;
    ...
    async fn test_concurrent_start_idempotent() {
        ...
        // Start twice rapidly
        let resp1 = ctx.post(&format!("/api/v1/apps/{app_id}/start"), json!({})).await;
        let resp2 = ctx.post(&format!("/api/v1/apps/{app_id}/start"), json!({})).await;

        // First should succeed, second should either succeed (idempotent)
        // or return 409 (already running)
        assert!(resp1.status().is_success());
        assert!(resp2.status().is_success() || resp2.status() == 409,
            "Concurrent start should be idempotent or rejected, got {}", resp2.status());
