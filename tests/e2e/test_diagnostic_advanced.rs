...
mod test_diagnostic_advanced {
    ...
    async fn test_all_8_diagnostic_matrix_combinations() {
        ...
        for (name, _, _, _, _) in &combos {
            ctx.post(&format!("/api/v1/apps/{app_id}/components"), json!({
                "name": name,
        ...
        let configs: Vec<(&str, i32, i32, i32)> = combos.iter()
            .map(|(name, h, i, f, _)| (*name, *h, *i, *f))
            .collect();
        ctx.configure_check_results(app_id, configs).await;

        // Run diagnostic
        let resp = ctx.post(&format!("/api/v1/apps/{app_id}/diagnose"), json!({})).await;
        assert_eq!(resp.status(), 200);
        let diag: Value = resp.json().await;
        ...
        for (name, _, _, _, expected) in &combos {
            let comp = components.iter()
                .find(|c| c["name"].as_str() == Some(name))
                .unwrap_or_else(|| panic!("Component {name} not found in diagnostic result"));
            assert_eq!(comp["recommendation"].as_str(), Some(*expected),
    ...
    async fn test_diagnose_returns_check_details() {
        ...
        ctx.configure_check_results(app_id, vec![
            ("Redis", 0, 0, 0),
            ("Tomcat", 2, 0, 0),
            ("Oracle", 2, 2, 0),
            ("Apache", 2, 2, 2),
        ...
        for comp in components {
            assert!(comp["health_check"].is_object() || comp["health_status"].is_string(),
                "Component should have health check details: {}", comp["name"]);
        }
    ...
    async fn test_rebuild_measures_rtr() {
        ...
        let resp = ctx.post(&format!("/api/v1/apps/{app_id}/rebuild"), json!({
            "components": [
        ...
        if resp.status().is_success() {
            ...
            if let Some(rtr) = result.get("rtr_seconds") {
                assert!(rtr.as_f64().unwrap_or(0.0) >= 0.0,
    ...
    async fn test_rebuild_with_site_override() {
        ...
        assert!(resp.status().is_success() || resp.status() == 200 || resp.status() == 202,
            "Rebuild with site override should be accepted, got {}", resp.status());
    ...
    async fn test_diagnose_requires_operate_permission() {
        ...
        let resp = ctx.post_as("viewer",
            &format!("/api/v1/apps/{app_id}/diagnose"), json!({})).await;
    ...
    async fn test_rebuild_requires_manage_permission() {
        ...
        let resp = ctx.post_as("operator",
            &format!("/api/v1/apps/{app_id}/rebuild"), json!({
    ...
    async fn test_diagnose_audit_trail() {
        ...
        ctx.post(&format!("/api/v1/apps/{app_id}/diagnose"), json!({})).await;
