...
async fn test_variable_crud() {
    ...
    // Delete variable
    let resp = ctx.delete_as("admin", &format!("/api/v1/apps/{app_id}/variables/{var_id}")).await;
...
async fn test_secret_variable_masking() {
    ...
    let resp = ctx.get_as("viewer", &format!("/api/v1/apps/{app_id}/variables")).await;
    assert_eq!(resp.status(), 200);
    let body: Value = resp.json().await;
...
async fn test_component_group_crud() {
    ...
    assert_eq!(resp.status(), 201);
    let group: Value = resp.json().await;
    ...
    // Delete group
    let resp = ctx.delete_as("admin", &format!("/api/v1/apps/{app_id}/groups/{group_id}")).await;
    assert_eq!(resp.status(), 204);
...
async fn test_component_links_crud() {
    ...
    // List links
    let resp = ctx.get(&format!("/api/v1/components/{comp_id}/links")).await;
    assert_eq!(resp.status(), 200);
    let body: Value = resp.json().await;
    ...
    // Delete link
    let resp = ctx.delete_as("admin", &format!("/api/v1/components/{comp_id}/links/{link_id}")).await;
    assert_eq!(resp.status(), 204);
...
async fn test_command_input_params_crud() {
    ...
    let resp = ctx.delete_as("admin", &format!("/api/v1/commands/{cmd_id}/params/{param_id}")).await;
    assert_eq!(resp.status(), 204);
