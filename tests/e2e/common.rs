...
impl TestContext {
    ...
    pub async fn new() -> Self {
        ...
        let db_url = format!(
            "postgres://appcontrol:test@localhost:5432/{db_name}"
        );
    ...
    pub async fn create_payments_app(&self) -> Uuid {
        let resp = self.post("/api/v1/apps", json!({
            "name": "Paiements-SEPA",
            "description": "SEPA payment processing",
            "tags": ["payments", "critical"]
        })).await;
        assert!(resp.status().is_success(), "Failed to create app: {}", resp.status());
        let app: Value = resp.json().await;
        let app_id: Uuid = app["id"].as_str().unwrap().parse().unwrap();

        let components = vec![
            ("Oracle-DB", "database", "check_oracle.sh", "start_oracle.sh", "stop_oracle.sh"),
            ("Tomcat-App", "appserver", "check_tomcat.sh", "start_tomcat.sh", "stop_tomcat.sh"),
            ("RabbitMQ", "middleware", "check_rabbitmq.sh", "start_rabbitmq.sh", "stop_rabbitmq.sh"),
            ("Apache-Front", "webfront", "check_apache.sh", "start_apache.sh", "stop_apache.sh"),
            ("Batch-Processor", "batch", "check_batch.sh", "start_batch.sh", "stop_batch.sh"),
        ];
        ...
        for (name, comp_type, check, start, stop) in &components {
            let resp = self.post(&format!("/api/v1/apps/{app_id}/components"), json!({
                "name": name,
        ...
        for (from, to) in &deps {
            self.post(&format!("/api/v1/apps/{app_id}/dependencies"), json!({
    ...
    pub async fn create_ten_component_app(&self) -> Uuid {
        ...
        let names = [
            "DB-1", "App-1", "Front-1", "Queue-1", "Worker-1",
            "DB-2", "App-2", "Front-2", "Queue-2", "Worker-2",
        ];
        let mut ids: std::collections::HashMap<String, Uuid> = std::collections::HashMap::new();
        for name in &names {
            let resp = self.post(&format!("/api/v1/apps/{app_id}/components"), json!({
        ...
        let deps = [
            ("DB-1", "App-1"), ("App-1", "Front-1"),
            ("App-1", "Queue-1"), ("Queue-1", "Worker-1"),
            ("DB-2", "App-2"), ("App-2", "Front-2"),
            ("App-2", "Queue-2"), ("Queue-2", "Worker-2"),
        ];
        for (from, to) in &deps {
            self.post(&format!("/api/v1/apps/{app_id}/dependencies"), json!({
                "from_component_id": ids[*from],
    ...
    pub async fn create_app_with_dr_sites(&self) -> (Uuid, Uuid, Uuid) {
        ...
        sqlx::query("INSERT INTO sites (id, organization_id, name, zone) VALUES ($1, $2, 'PRD', 'PRD')")
            .bind(site_a).bind(self.organization_id).execute(&self.db_pool).await.unwrap();
        sqlx::query("INSERT INTO sites (id, organization_id, name, zone) VALUES ($1, $2, 'DR', 'DR')")
            .bind(site_b).bind(self.organization_id).execute(&self.db_pool).await.unwrap();
        ...
        for (site_id, suffix) in [(site_a, "prd"), (site_b, "dr")] {
            for name in ["Oracle-DB", "Tomcat-App", "Apache-Front"] {
                self.post(&format!("/api/v1/apps/{app_id}/components"), json!({
    ...
    pub async fn create_payments_app_with_checks(&self) -> Uuid {
        ...
        for (name, comp_type) in [("Redis", "cache"), ("Tomcat", "appserver"), ("Oracle", "database"), ("Apache", "webfront")] {
            self.post(&format!("/api/v1/apps/{app_id}/components"), json!({
                "name": name,
                "component_type": comp_type,
                "hostname": format!("srv-{}", name.to_lowercase()),
    ...
    pub async fn set_all_running(&self, app_id: Uuid) {
        sqlx::query("UPDATE components SET state = 'RUNNING' WHERE application_id = $1")
            .bind(app_id).execute(&self.db_pool).await.unwrap();
    }

    pub async fn set_all_running_on_site(&self, app_id: Uuid, site_id: Uuid) {
        sqlx::query("UPDATE components SET state = 'RUNNING' WHERE application_id = $1 AND site_id = $2")
            .bind(app_id).bind(site_id).execute(&self.db_pool).await.unwrap();
    }

    pub async fn get_app_status(&self, app_id: Uuid) -> AppStatus {
        let resp = self.get(&format!("/api/v1/orchestration/apps/{app_id}/status")).await;
        resp.json().await
    }
    ...
    pub async fn force_component_state(&self, app_id: Uuid, name: &str, state: &str) {
        sqlx::query("UPDATE components SET state = $1 WHERE application_id = $2 AND name = $3")
            .bind(state).bind(app_id).bind(name).execute(&self.db_pool).await.unwrap();
    }
    ...
    pub async fn component_id(&self, app_id: Uuid, name: &str) -> Uuid {
        ...
        .bind(app_id).bind(name).fetch_one(&self.db_pool).await.unwrap()
    }

    pub fn component_state<'a>(&self, status: &'a AppStatus, name: &str) -> &'a str {
        status.components.iter()
            .find(|c| c.name == name)
    ...
    pub async fn wait_app_running(&self, app_id: Uuid, timeout: Duration) -> anyhow::Result<()> {
        ...
        loop {
            ...
            if status.components.iter().any(|c| c.state == "FAILED") {
                anyhow::bail!("Component failed: {:?}",
                    status.components.iter().find(|c| c.state == "FAILED").unwrap().name);
            }
    ...
    pub async fn wait_app_branch_running(&self, app_id: Uuid, timeout: Duration) -> anyhow::Result<()> {
        let deadline = tokio::time::Instant::now() + timeout;
        loop {
            let status = self.get_app_status(app_id).await;
            if !status.components.iter().any(|c| c.state == "FAILED" || c.state == "STARTING") {
                return Ok(());
            }
            if tokio::time::Instant::now() > deadline {
    ...
    pub async fn get_check_events(&self, component_id: Uuid) -> Vec<CheckEvent> {
        ...
        .bind(component_id).fetch_all(&self.db_pool).await.unwrap()
    }
    ...
    pub async fn set_component_check_will_fail(&self, app_id: Uuid, name: &str) {
        ...
        .bind(app_id).bind(name).execute(&self.db_pool).await.unwrap();
    }
    ...
    pub async fn grant_permission(&self, app_id: Uuid, user_id: Uuid, level: &str) {
        self.post_as("admin", &format!("/api/v1/apps/{app_id}/permissions/users"),
            json!({"user_id": user_id, "permission_level": level})).await;
    }

    pub async fn grant_permission_with_expiry(
        &self, app_id: Uuid, user_id: Uuid, level: &str,
    ...
    pub async fn create_command(&self, component_id: Uuid, name: &str, cmd: &str, confirm: bool) {
        self.put(&format!("/api/v1/components/{component_id}"), json!({
            "commands": [{
                "name": name,
                "display_name": name,
                "command": cmd,
    ...
    pub async fn create_api_key(&self, name: &str, actions: Vec<&str>) -> String {
        let resp = self.post("/api/v1/api-keys", json!({
            "name": name,
            "allowed_actions": actions,
        })).await;
        let key: Value = resp.json().await;
    ...
    pub async fn disconnect_agent(&self, hostname: &str) {
        sqlx::query("UPDATE components SET state = 'UNREACHABLE' WHERE hostname = $1")
            .bind(hostname).execute(&self.db_pool).await.unwrap();
    }
    ...
    pub async fn create_second_org(&self) -> (Uuid, Uuid, String) {
        ...
        sqlx::query("INSERT INTO organizations (id, name) VALUES ($1, 'Other Org')")
            .bind(org2_id).execute(&self.db_pool).await.unwrap();
        sqlx::query(
            "INSERT INTO users (id, organization_id, username, display_name, role, email)
             VALUES ($1, $2, 'other_admin', 'Other Admin', 'admin', 'other@test.local')"
        )
        .bind(user2_id).bind(org2_id).execute(&self.db_pool).await.unwrap();
