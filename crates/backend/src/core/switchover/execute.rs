...
pub(crate) async fn execute_stop_source(
    ...
    if mode == "SELECTIVE" {
        if let Some(ids) = &component_ids {
            if !ids.is_empty() {
                ...
                for &comp_id in ids {
                    impacted_set.extend(dag.find_all_dependents(comp_id));
                }
    ...
    Ok(
        serde_json::json!({"source_stopped": true, "mode": mode, "components_still_running": still_running}),
    )
}
...
pub(crate) async fn execute_start_target(
    ...
    for (comp_name, new_agent_id) in &mappings {
        ...
        if let Some((comp_id, cur_agent, cur_check, cur_start, cur_stop)) = comp_info {
            ...
            repo::update_component_for_switchover(
                &state.db,
                comp_id,
                new_agent_id,
                &check_override,
            ...
            let _ = repo::record_switchover_config_version(
                &state.db,
                comp_id,
                initiated_by,
                before,
