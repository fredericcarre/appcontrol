...
pub(crate) async fn execute_validate(
    ...
    let (target_profile_id, target_profile_name) = match target_profile {
        Some((id, name, count)) => {
            if count == 0 { issues.push(format!("Binding profile '{}' has no component mappings", name)); }
            (id, name)
        }
        None => {
            issues.push(format!("No binding profile found for target site (site_id={})", target_site_id));
            return Err(SwitchoverError::ValidationFailed(serde_json::to_string(&issues).unwrap_or_default()));
        }
    };
    ...
    for (agent_id, hostname) in &target_agents {
        ...
        match last_heartbeat {
            Some(hb) if (chrono::Utc::now() - hb).num_seconds() > 120 =>
                issues.push(format!("Agent '{}' ({}) last heartbeat was {}s ago (stale)", hostname, agent_id, (chrono::Utc::now() - hb).num_seconds())),
            None => issues.push(format!("Agent '{}' ({}) has never sent a heartbeat", hostname, agent_id)),
            _ => {}
        }
    }
