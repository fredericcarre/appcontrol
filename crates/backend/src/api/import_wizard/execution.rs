...
pub async fn execute_import(
    ...
    // Handle conflicts
    let existing_app =
        repo::find_app_by_name(&state.db, *user.organization_id, &original_name).await?;
    ...
    let (app_id, app_name, is_update) = match (&body.conflict_action, existing_app) {
        ...
        (ConflictAction::Rename, Some(_)) => {
            ...
            let new_exists =
                repo::find_app_by_name(&state.db, *user.organization_id, &new_name).await?;
            if new_exists.is_some() {
                return Err(ApiError::Conflict(format!(
                    "Application '{}' also already exists.",
                    new_name
                )));
            }
    ...
    for name in &component_names {
        if !mapped_names.contains(&name) {
            return Err(ApiError::Validation(format!(
                "Component '{}' has no agent mapping.",
                name
            )));
        }
    }
    ...
    if is_update {
        repo::update_application_for_import(
            &state.db,
            app_id,
            import_data.application.description.as_deref(),
            site_id,
            &tags_json,
        )
        .await?;
        ...
        repo::insert_application_for_import(
            &state.db,
            app_id,
            &app_name,
            import_data.application.description.as_deref(),
    ...
    for var in &import_data.application.variables {
        repo::create_app_variable_with_secret(
            &state.db,
            app_id,
            &var.name,
            &var.value,
    ...
    for (idx, group) in import_data.application.groups.iter().enumerate() {
        ...
        repo::create_component_group_full(
            &state.db,
            group_id,
            app_id,
            &group.name,
    ...
    for (idx, comp) in import_data.application.components.iter().enumerate() {
        ...
        repo::insert_import_component_with_agent(
            ...
            comp.host.as_deref(),
            agent_id,
            check_cmd.map(|s| s.as_str()),
            start_cmd.map(|s| s.as_str()),
            stop_cmd.map(|s| s.as_str()),
            integrity_cmd.map(|s| s.as_str()),
            post_start_cmd.map(|s| s.as_str()),
            infra_cmd.map(|s| s.as_str()),
            rebuild_cmd.map(|s| s.as_str()),
            rebuild_infra_cmd.map(|s| s.as_str()),
        ...
        for (lidx, link) in comp.links.iter().enumerate() {
            repo::create_component_link_ordered(
                &state.db,
                comp_id,
                &link.label,
                &link.url,
        ...
        for override_data in &comp.site_overrides {
            let site_row =
                repo::find_site_by_code(&state.db, *user.organization_id, &override_data.site_code)
                    .await?;
            ...
            repo::upsert_site_override(
                ...
                override_site_id,
                agent_id_override,
                override_data.check_cmd_override.as_deref(),
                override_data.start_cmd_override.as_deref(),
                override_data.stop_cmd_override.as_deref(),
                override_data.rebuild_cmd_override.as_deref(),
    ...
    repo::create_binding_profile(
        ...
        app_id,
        &body.profile.name,
        body.profile.description.as_deref(),
        &body.profile.profile_type,
        true,
        UuidArray::from(body.profile.gateway_ids.clone()),
        body.profile.auto_failover.unwrap_or(false),
        *user.user_id,
    )
    .await?;
    ...
    for mapping in &body.profile.mappings {
        let host = import_data
            ...
            .unwrap_or_default();
        repo::create_binding_profile_mapping(
            &state.db,
            primary_profile_id,
    ...
    for dr_profile in &all_dr_profiles {
        ...
        repo::create_binding_profile(
            ...
            app_id,
            &dr_profile.name,
            dr_profile.description.as_deref(),
            &dr_profile.profile_type,
            false,
            UuidArray::from(dr_profile.gateway_ids.clone()),
            dr_profile.auto_failover.unwrap_or(false),
            *user.user_id,
        )
        .await?;
