...
// ============================================================================
// Database-specific helper functions — delegated to repository
// ============================================================================

type GlobalAuditRow = crate::repository::report_queries::GlobalAuditRow;
...
async fn fetch_global_audit_logs(
    ...
    crate::repository::report_queries::fetch_global_audit_logs(db, org_id, app_id, user_id, limit, offset).await
}
...
async fn fetch_availability_stats(
    ...
    crate::repository::report_queries::fetch_availability_stats(db, app_id, from, to).await
}
...
async fn fetch_switchover_logs(
    ...
    crate::repository::report_queries::fetch_switchover_logs(db, app_id).await
}
...
async fn fetch_topology_components(
    ...
    crate::repository::report_queries::fetch_topology_components(db, app_id).await
}

async fn fetch_avg_rto(db: &crate::db::DbPool, app_id: Uuid) -> Option<f64> {
    crate::repository::report_queries::fetch_avg_rto(db, app_id).await
}
...
async fn fetch_availability_summary(
    ...
    crate::repository::report_queries::fetch_availability_summary(db, app_id, from, to).await
}
...
async fn fetch_mttr_recoveries(
    ...
    crate::repository::report_queries::fetch_mttr_recoveries(db, app_id, from, to).await
}

// Old implementations removed — now delegated to repository::report_queries
