...
pub async fn toggle_schedule(
    ...
    let row = sched_repo::fetch_schedule_row(&state.db, schedule_id)
        .await.map_err(|e| ApiError::Internal(e.to_string()))?.ok_or(ApiError::NotFound)?;

    let app_id = if let Some(aid) = row.application_id { aid }
    else if let Some(cid) = row.component_id {
        get_app_id_for_component(&state.db, cid).await.ok_or(ApiError::NotFound)?
    } else { return Err(ApiError::Internal("Invalid schedule target".to_string())); };

    let perm = effective_permission(&state.db, user.user_id, app_id, user.is_admin()).await;
    if perm < PermissionLevel::Operate { return Err(ApiError::Forbidden); }

    let new_enabled = !row.is_enabled;
    let next_run_at = if new_enabled { calculate_next_run(&row.cron_expression, &row.timezone) } else { None };

    let _ = audit::log_action(&state.db, user.user_id,
        if new_enabled { "enable_schedule" } else { "disable_schedule" },
        "schedule", schedule_id, serde_json::json!({"schedule_name": row.name}),
...
pub async fn run_schedule_now(
    ...
    let _ = audit::log_action(&state.db, user.user_id, "run_schedule_now", "schedule", schedule_id,
        serde_json::json!({"schedule_name": row.name, "operation": row.operation}),
    ).await.map_err(|e| ApiError::Internal(e.to_string()))?;

    Ok(Json(serde_json::json!({"message": "Schedule queued for immediate execution", "schedule_id": schedule_id})))
}
...
pub async fn list_schedule_executions(
    ...
    let responses = executions.into_iter().map(|e| ScheduleExecutionResponse {
        id: e.id, schedule_id: e.schedule_id, action_log_id: e.action_log_id,
        executed_at: e.executed_at, status: e.status, message: e.message, duration_ms: e.duration_ms,
    }).collect();
