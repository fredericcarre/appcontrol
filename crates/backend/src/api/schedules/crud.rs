...
pub async fn list_component_schedules(
    ...
    let include_disabled = query.include_disabled.unwrap_or(false);
    let rows = if include_disabled {
        sched_repo::list_component_schedules_all(&state.db, comp_id)
            .await
            .map_err(|e| ApiError::Internal(e.to_string()))?
    } else {
        sched_repo::list_component_schedules_enabled(&state.db, comp_id)
            .await
            .map_err(|e| ApiError::Internal(e.to_string()))?
    };
...
pub async fn create_component_schedule(
    ...
    let schedule_id = Uuid::new_v4();
    sched_repo::create_component_schedule(
        &state.db,
        schedule_id,
        org_id,
        comp_id,
...
pub async fn update_schedule(
    ...
    sched_repo::update_schedule_fields(
        &state.db,
        schedule_id,
        &name,
        &description,
...
async fn resolve_app_id(
    db: &crate::db::DbPool,
    row: &ScheduleRow,
) -> Result<crate::db::DbUuid, ApiError> {
    if let Some(aid) = row.application_id {
        Ok(aid)
    } else if let Some(cid) = row.component_id {
        get_app_id_for_component(db, cid)
            .await
            .ok_or(ApiError::NotFound)
    } else {
