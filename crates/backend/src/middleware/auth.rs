...
async fn is_token_revoked(pool: &crate::db::DbPool, token: &str) -> bool {
    ...
    match crate::repository::auth_queries::is_token_revoked(pool, &fingerprint).await {
        Ok(revoked) => revoked,
        Err(e) => {
...
pub async fn revoke_token(pool: &crate::db::DbPool, token: &str) -> Result<(), String> {
    ...
    crate::repository::auth_queries::revoke_token_by_fingerprint(pool, &fingerprint, ttl_secs)
        .await
...
pub async fn cleanup_expired_revocations(pool: &crate::db::DbPool) {
    match crate::repository::auth_queries::cleanup_expired_revocations(pool).await {
        Ok(cleaned) if cleaned > 0 => {
            tracing::debug!(cleaned, "Cleaned expired token revocations");
        }
        Err(e) => {
