...
pub async fn get_app_info_for_report(
    db: &DbPool,
    app_id: Uuid,
) -> Result<Option<(String, Option<DbUuid>, Option<String>)>, sqlx::Error> {
...
pub async fn get_switchover_log_entries(
    db: &DbPool,
    app_id: Uuid,
...
pub async fn get_transitions_in_range(
    ...
    let sql = format!(
        ...
        JOIN components c ON c.id = st.component_id WHERE c.application_id = $1
          AND st.created_at >= $2 AND st.created_at <= $3 AND st.to_state IN ({}) ORDER BY st.created_at ASC"#,
        states_str
    );
...
pub async fn get_app_dependencies(
    db: &DbPool,
    app_id: Uuid,
...
pub async fn get_error_components(
    db: &DbPool,
    app_id: Uuid,
) -> Result<Vec<(DbUuid, String, String, chrono::DateTime<chrono::Utc>)>, sqlx::Error> {
    sqlx::query_as::<_, (DbUuid, String, String, chrono::DateTime<chrono::Utc>)>(
...
pub async fn get_app_agents(
    db: &DbPool,
    app_id: Uuid,
) -> Result<Vec<(DbUuid, String, bool, Option<chrono::DateTime<chrono::Utc>>)>, sqlx::Error> {
