...
ALTER TABLE applications ADD COLUMN IF NOT EXISTS is_suspended BOOLEAN NOT NULL DEFAULT false;
ALTER TABLE applications ADD COLUMN IF NOT EXISTS suspended_at TIMESTAMPTZ;
ALTER TABLE applications ADD COLUMN IF NOT EXISTS suspended_by UUID REFERENCES users(id);

-- Index for filtering active applications efficiently
CREATE INDEX IF NOT EXISTS idx_applications_suspended ON applications (organization_id) WHERE is_suspended = false;
